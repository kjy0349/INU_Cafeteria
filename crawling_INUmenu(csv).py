...
all_menu_clist = []
for i in range(len(all_menu_list)):
    for j in range(len(all_menu_list[i])):
        for k in range(len(all_menu_list[i][j])):
            temp = all_menu_list[i][j][k]
            all_menu_clist.append(temp)

menu_df = pd.DataFrame(all_menu_clist)
menu_df.to_csv("menu.csv", header=False, index=False)