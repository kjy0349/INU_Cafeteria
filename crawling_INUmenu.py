...
today_menu_list = [[]] # 하루당 총 메뉴 개수 24개, 1일
not_today_menu_list = [] # 당일 메뉴를 제외한 6일
...
def find_td(selected_menu, result_list, index):
        ...
        for j in range(len(oneday_menuplan)):
            if (oneday_menuplan[j] is '\r' or  # 메뉴가 게재된칸에는 가격정보와 제공시간이 적혀있으므로, 메뉴만 얻기위해 예외처리
                ...
                if '국밥' in oneday_menuplan[j]:  # 국밥칸에서 칼로리 정보를 제외하고 메뉴 이름만 가져오기위한 예외처리
                    result_list[index].append(oneday_menuplan[j][:4].strip())
                else:
                    result_list[index].append(oneday_menuplan[j].strip())
...
for tr_menu in soup.find(id='menuBox').find_all('tr'):  # menuBox라는 이름을 가진 테이블에서 한줄씩 뽑아냄
    ...
    if not_today_menu:  # 찾은 내용이 있을 경우
        find_td(not_today_menu, not_today_menu_list, temp)
    if today_menu:
        find_td(today_menu, today_menu_list, 0)
    temp += 1

for i in range(len(not_today_menu_list)): # 토,일 메뉴 제거 ❝ 해당 기호 처리 안됨
    if not_today_menu_list[i]:
        not_today_menu_list[i].pop()
        not_today_menu_list[i].pop()

for i in range(len(not_today_menu_list)):
    for j in range(len(not_today_menu_list[i])):
        print(not_today_menu_list[i][j], end=' ')
    print(' ')
